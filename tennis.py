...
class Tennis(Sport):
    ...
    def lineCall(self, xy_In, frame, prerecorded_flag, camera_mode):
        if self.court_reference is not None:
            return self.computeLineCall(xy_In=xy_In, frame=frame, court_lines = self.court_reference, custom_flag=0, prerecorded_flag=prerecorded_flag, camera_mode=camera_mode)
        elif self.custom_lines is not None:
            # use the custom lines...
            return self.computeLineCall(xy_In=xy_In, frame=frame, court_lines = self.custom_lines, custom_flag=1, prerecorded_flag=prerecorded_flag, camera_mode=camera_mode)
        # return the frame no matter what, if niether condition wasnt met then its just the same frame that came in
        return frame

    def computeLineCall(self, xy_In, frame, court_lines, custom_flag, prerecorded_flag, camera_mode):
